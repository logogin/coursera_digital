...
or = imread('original_cameraman.jpg');
original = im2double((imread('original_cameraman.jpg')));
[H, W] = size(original);
...
blurred = real(ifft2(blurred_freq));
blurred = blurred(1 : H, 1 : W);
blurred(blurred < 0) = 0;
blurred(blurred > 1) = 1;
im = mat2gray(blurred, [0, 255]);
...
_isnr = isnr(original, noisy, restored);
